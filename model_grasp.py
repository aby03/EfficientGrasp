...
class GraspNet(models.Model):
    def __init__(self, width, depth, num_anchors=1, separable_conv=True, freeze_bn=False, detect_quadrangle=False, **kwargs):
...
def efficientgrasp(phi, num_classes=20, num_anchors=9, weighted_bifpn=False, freeze_bn=False,
    ...
    grasp_regression = [grasp_net([feature, i]) for i, feature in enumerate(fpn_features)]
    ...
    # model = models.Model(inputs=[image_input], outputs=[classification, regression, rotation, translation], name='efficientgrasp')
    model = models.Model(inputs=[image_input], outputs=[grasp_regression], name='efficientgrasp')
    ...
    all_layers = list(set(model.layers + grasp_net.layers))
    ...
    prediction_model = models.Model(inputs=[image_input], outputs=[grasp_regression], name='efficientgrasp_p')
    return model, prediction_model, all_layers


if __name__ == '__main__':
    x, y, _ = efficientgrasp(0)
